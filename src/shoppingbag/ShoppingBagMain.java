...
public class ShoppingBagMain 
{
	public static void main(String[] args)
	{
		Scanner input = new Scanner(System.in);
		ShoppingBag myShoppingBag = null;
		int numItems;
		int choice;
		...
		if(slct == 2)
		{
			try
			{
				ObjectInputStream ois = new ObjectInputStream(new FileInputStream("shoppingbag.ser"));
				myShoppingBag = (ShoppingBag) ois.readObject();
		...
		else
		{
			do
			{
				System.out.print("About how many items do you plan to place in the shopping bag? ");
				numItems = input.nextInt();
				input.nextLine();
				try
				{
					myShoppingBag = new ShoppingBag(numItems);
				}
				catch(InvalidDataException e)
				{
					System.out.println(e);
					System.out.println("You must enter a positive number for the number of items");
				}
			}while(numItems < 1);
		}
		...
		{		
			...
			switch(choice)
			{
				...
				case 2:
					...
					try
					{	
						myShoppingBag.removeItem(description);
					...
					break;
				case 3:
					System.out.println(myShoppingBag.toString());
					break;
				case 4:
					System.out.println("Exiting application.");
					try
					{
						ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("shoppingbag.ser"));
						oos.writeObject(myShoppingBag);
						System.out.println("All data is saved to shoppingbag.ser.");
						System.exit(0);
					}
					catch(IOException e)
					{
